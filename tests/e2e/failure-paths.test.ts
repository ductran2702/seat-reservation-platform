...
describe("payment failure paths", () => {
  ...
  it("confirm?outcome=timeout stays HELD; retry with success confirms", async () => {
    ...
    try {
      ...
      const timeoutBody = (await timeout.json()) as {
        reservation: { status: string };
      };
      expect(timeoutBody.reservation.status).toBe("HELD");
      ...
      expect(successBody.reservation.status).toBe("CONFIRMED");
    } finally {
      await cleanupSeat(seat.id);
    }
  });
  ...
  it("intent on an expired hold → 409 hold_expired", async () => {
    ...
    try {
      ...
      const intentBody = (await intent.json()) as { error: string };
      expect(intentBody.error).toBe("hold_expired");
    } finally {
      await cleanupSeat(seat.id);
    }
  });
...
describe("hold lifecycle", () => {
  it("DELETE cancels a HELD reservation and frees the seat", async () => {
    ...
    try {
      ...
      expect(delBody.reservation.status).toBe("CANCELLED");
